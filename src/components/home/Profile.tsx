...
export default function Profile({ author, social, features, researchInterests }: ProfileProps) {
    ...
    useEffect(() => {
        ...
        const userHasLiked = localStorage.getItem('kaiyi-website-user-liked');
    ...
    const handleLike = () => {
        ...
        if (newLikedState) {
            localStorage.setItem('kaiyi-website-user-liked', 'true');
            setShowThanks(true);
            setTimeout(() => setShowThanks(false), 2000);
        } else {
            localStorage.removeItem('kaiyi-website-user-liked');
            setShowThanks(false);
        }
    };
